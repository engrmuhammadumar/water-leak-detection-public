...
def measure_latency(model: torch.nn.Module, sample_seq: torch.Tensor, device="cpu", warmup=5, runs=50) -> float:
...
def slice_time_axis(img_t: torch.Tensor, seq_len: int, out_hw: Tuple[int,int]) -> torch.Tensor:
    ...
    if W < seq_len:
        ...
        img_t = Fnn.interpolate(img_t.unsqueeze(0), size=(H, seq_len),
...
def _rand_mask_1d(length: int, max_frac: float):
    w = max(1, int(round(length * max_frac)))
    if w >= length: return 0, length
    import random as _r
    s = _r.randint(0, length - w)
    return s, s + w
...
def spec_augment_(img_chw: torch.Tensor, time_masks: int, time_w: float,
    ...
    for _ in range(time_masks):
        xs, xe = _rand_mask_1d(W, time_w); img_chw[:, :, xs:xe] = 0.0
    for _ in range(freq_masks):
        ys, ye = _rand_mask_1d(H, freq_h); img_chw[:, ys:ye, :] = 0.0
